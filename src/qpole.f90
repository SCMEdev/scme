...
SUBROUTINE get_quadrupoles(cec,cer2,rCE,quad,nM)
    integer,parameter    :: xyz=3,hho=3
    integer, intent(in) :: nM
    real(dp),intent(in) :: rCE(xyz,nM), cec(xyz,hho,nM),cer2(hho,nM) 
    real(dp),intent(out) :: quad(xyz,xyz,nM)
    
    !internal
    real(dp) p(hho),dpda(xyz,hho,hho)
    integer m
    
    do m = 1, nM
      call quad_charges(cec(:,:,m),cer2(:,m), p, dpda)
      call quad_tensor(cec(:,:,m),cer2(:,m),p,quad(:,:,m))
      !call quad_derivatives(dpda)
    enddo
...
SUBROUTINE quad_charges(cec,cer2, p , dpda)
    integer,parameter    :: xyz=3,hho=3, rra=3
    real(dp),intent(in)  :: cec(xyz,hho),cer2(hho)
    real(dp),intent(out) :: p(hho), dpda(xyz,hho,hho) !right to left: deriviv of charge on atom (hho) w.r.t. position of atom (hho) in direction (xyz). 
    real(dp) hx2
