...
!JÖ internal:     
    ...
    real(dp) d(3), q(3,3), o(3,3,3), h(3,3,3,3)
    ...
    do i = 1, 3
       ...
       do j = i, 3
          ...
          do k = j, 3
             ...
             !     3rd derivative
             dt1k = 6.0_dp * o(i,j,k)
             dt2k = 3.0_dp * (v(i)*djk + v(j)*dik + v(k) * dij) + 6.0_dp &
                  * (g(i,k)*r(j) + g(j,k)*r(i))
             ...
             do l = k, 3
                ...
                !     4th derivative
                dt1l = 6.0_dp * o(i,j,l)
                ddt2kl = 6.0_dp * (g(i,l)*djk + g(j,l)*dik + g(k,l) &
                     * dij) + 6.0_dp * (o(i,k,l)*r(j) + o(j,k,l)*r(i)) &
                ...
                do s = l, 3
                   ...
                   dddt3kls = 6.0_dp * (o(k,l,s)*r(i)*r(j) + g(k,l) &
                        * (dis*r(j) + r(i)*djs)) &
                   ...
                   ddt3ks = 6.0_dp * g(k,s)*r(i)*r(j) + 3.0_dp * v(k) &
                        * (dis*r(j) + r(i)*djs) &
                   ...
                   ddt3ls = 6.0_dp * g(l,s)*r(i)*r(j) + 3.0_dp * v(l) &
                        * (dis*r(j) + r(i)*djs) &
    ...
    do i = 1, 3
       ...
       do j = i, 3
          ...
          do k = j, 3
             ...
             dt2k = 4.0_dp * (v(i)*djk + v(j)*dik + v(k) * dij) + 12.0_dp &
                  * (g(i,k)*r(j) + g(j,k)*r(i))
             ...
             do l = k, 3
                ...
                ddt2kl = 12.0_dp * (g(i,l)*djk + g(j,l)*dik + g(k,l) &
                     * dij) + 24.0_dp * (d(i,k,l)*r(j) + d(j,k,l)*r(i)) &
                ...
                do s = l, 3
                   ...
                   dddt2kls = 24.0_dp * (d(i,l,s)*djk + d(j,l,s)*dik + &
                        d(k,l,s) * dij) + 24.0_dp * (h(i,k,l,s)*r(j) + &
                   ...
                   ddt2ks = 12.0_dp * (g(i,s)*djk + g(j,s)*dik + g(k,s) &
                        * dij) + 24.0_dp * (d(i,k,s)*r(j) + d(j,k,s)*r(i)) + &
                   ...
                   dddt3kls = 12.0_dp * (2.0_dp * d(k,l,s)*r(i)*r(j) + g(k,l) &
                        * (dis*r(j) + r(i)*djs) ) &
                        + 4.0_dp * (3.0_dp * g(k,s) * (dil*r(j) + r(i)*djl ) + &
                        v(k) * (dil*djs + dis*djl) )+ 4.0_dp * v(s) &
                        * (dil*djk + djl*dik)+ 4.0_dp * (v(l) * (dis*djk + djs*dik) &
                   ...
                   ddt3ks = 4.0_dp * (3.0_dp * g(k,s)*r(i)*r(j)  &
                        + v(k) * (dis*r(j) + r(i)*djs) )  &
                        + r4h * (dis*djk + djs*dik)  &
                   ...
                   ddt3ls = 4.0_dp * (3.0_dp * g(l,s)*r(i)*r(j)  &
                        + v(l) * (dis*r(j) + r(i)*djs) )  &
                        + r4h * (dis*djl + djs*dil)  &
